...
%
\revision{}{(1)} questionnaire \revision{survey}{surveys} asking MPI users questions
specifically to reflect the human understanding or
...
\subsection*{Design}
%
Poole and Ahmed, our consulting social
scientists, suggested that the number of questions must be
limited to
around 30, to keep participants engaged. This number is significantly smaller that those of ECP and HPCI surveys,
...
community such as \url{hpc-announce@mcs.anl.gov}, but the contributions were
...
correlation between the two questions. The cross-tab heatmap graphs of
...
\revision{Table~\ref{tab:multi-thread-raw}}{Fig.~\ref{fig:multi-thread-raw}} shows the top \revision{7}{seven} answers
...
auto-evaluate their programming skills higher (those who chose 4 or
...
order to slow the pace of introducing new features, and to make sure these new additions
are needed by the MPI user community} and
%
...
As described, we follow the advice of social scientists and we have
...
question is from zero (Q2) to 22\% (Q11, asking reading MPI
...
this is the narrow coverage of wide spectrum of MPI topics.

\item[Mailing List]
First, the mailing list (ML) for announcing a survey must be selected
in such a way that the ML subscribers and the target of the survey
must be matched. Second, try using minor mailing lists (MLs) to
collect answers. Most major MLs such as {\tt hpc-announce} have lots
