...
\item[Simple Questionnaire]
As described, we follow the advice of social scientists and we have
only 30 questions which is much smaller than that of precedent
surveys (Section~\ref{sec:background}). The ``no answer'' rate of each
question is from zero (Q2) to 22\% (Q11). Especially, this rate of the 
last question is only 2\% (18). This indicates that the questionnaire
design was successful without having many withdrawals. 
...
online form you use can be accessed without any obstacles. Otherwise
the number of answers might be biased by countries.

\item[Reconsideration]
One big reconsideration is that we should have had the question asking
age ranges of the participants. This would have revealed if MPI users
of a country or region consists of what generations.
