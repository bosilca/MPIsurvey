...
\affiliation[2]{organization={INRIA Bordeaux Sud-Ouest},
  addressline={200, Avenue de la Vielle Tour 33405 Talence Cedex},
  city={Bordeaux},
  postcode={????},
  country={France}
}  
...
\begin{abstract}
  ...
  users' status, concerns, dissatisfaction, and expectations, and adapts
...
So far, there are three MPI survey classes; {\em Q} questionnaire
survey asking questions to MPI users such as ours and the ECP survey,
{\em S} analyzing MPI program statically, and {\em R} analyzing MPI behavior
at runtime by using a profiling tool. Apparently our survey and the
ECP survey are categorized in {\em Q}. The surveys categorized as
{\em S} are
\cite{https://doi.org/10.1002/cpe.5901}. The {\em R}
class surveys are
\cite{8665758,10.1007/978-3-319-58667-0-12}. 

In the {\em S} category, \cite{10.1145/3295500.3356176} statically
investigated 110 open-source MPI
programs. \cite{https://doi.org/10.1002/cpe.5901} investigated 14 MPI
programs chosen from the ECP Proxy Applications Suite
2.0~\cite{osti-1482870}. 

In the {\em R} category, \cite{8665758} collected and
analyzed the runtime behavior by running more than 100K MPI
jobs. \cite{10.1007/978-3-319-58667-0-12} analyzed behavior of DOE
...
rows (1 is the lowest skill and 6 is the highest skill, in this case)
in this figure are 
...
\myquote{What are your obstacles to mastering MPI?} is　
...
Although the largest number of answers is
...
Hoefler et al., reported their idea to extract collective operations
from a series of communication primitives, send and
receive\cite{7842939}. Although their idea is at the experimental
stage, however, this seems to be a good solution not introducing new
functions and narrowing the gap.
...
as described in Subsection~\ref{sec:learning-mpi}. This suggests that
...
\begin{enumerate*}
\item slow the pace of introducing new features, and
\item create a new working group to prepare and maintain web pages for
